...
export default class Contact extends Vue {
  ...
  async create_contact() {
    this.gql_op('create_contact', 'CreateContact', 'create');
  }

  async update_contact() {
    this.gql_op('update_contact', 'UpdateContact', 'update');
  }

  private async gql_op(operation: string, param_type: string, event: string) {
    this.error = '';
    try {
      this.is_loading = true;
      ...
      this.$emit(event, data[operation]);
      this.close();
    } catch (err) {
      this.error = err.message;
    } finally {
