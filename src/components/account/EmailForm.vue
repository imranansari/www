    ...
    <v-card>
      ...
      <v-card-text>
        <v-container>
          ...
          outline
          mask="####-####"
          @keyup="check_confirmation_code_length"
          ></v-text-field>
        </v-container>
      </v-card-text>
      <v-card-actions>
        ...
        <v-btn
        color="primary"
        :disabled="confirmation_code.length !== 8"
        :loading="is_loading"
        @click="verify">
          Confirm
        </v-btn>
...
export default class EmailForm extends Vue {
    ...
    try {
      ...
      const data = await api.mutate(
        ...
        `mutation ($email: String!) {
          request_email_update(email: $email)
        }
        `,
        {
      ...
      this.pending_email_id = data.request_email_update;
  ...
  check_confirmation_code_length() {
    if (this.confirmation_code.length === 8) {
      this.verify();
    }
  }
  ...
  async verify() {
    try {
      ...
      const data = await api.mutate(
        api.ACCOUNT_GRAPHQL,
        `mutation ($update: EmailUpdate!) {
          update_email(update: $update) {
            email
          }
        }
        `,
        {
          update: { id: this.pending_email_id, token: this.confirmation_code },
        },
      );
      this.close_dialog();
      this.$toast.success('Success', { icon: 'mdi-check-circle' });
      this.$emit('update', data.update_email.email);
      this.$store.commit('set_account', data.update_email);
    } catch (err) {
